...
export function Portfolio() {
  const { isConnected } = useWallet();  const {
    fetchBalances
  } = useRealDEX();
  ...
  const [selectedChain, setSelectedChain] = useState('all'); // 'all' or chain index
            ...
            <div className="grid gap-4 md:gap-6">
                  ...
                  <SelectContent>
                    ...
                    {portfolioData.chains.map((chain, index) => (
                      <SelectItem key={index} value={index.toString()}>
                        <div className="flex items-center gap-2">
                          <span className={chain.color}>{chain.logo}</span>
              ...
              <div className="grid gap-4">
                {selectedChain === 'all' ? (
                  ...
                  // Show specific chain
                  (() => {
                    const chainIndex = parseInt(selectedChain);
                    const chain = portfolioData.chains[chainIndex];
                    const totalChainValue = chain.balances.reduce((sum, balance) => sum + balance.usdValue, 0);
                      ...
                      <Card>
                        ...
                        <CardContent className="pt-0">
                          <div className="space-y-3">
                            {chain.balances.map((balance, balanceIndex) => (
                              <div key={balanceIndex} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    {balance.isNative && (
                                      <Badge variant="outline" className="text-xs mt-1">Native Token</Badge>
                                    )}
                                  </div>
                                </div>
                                <div className="text-right">
